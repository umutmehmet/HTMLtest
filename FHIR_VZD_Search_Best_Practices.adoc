...
|https://gemspec.gematik.de/docs/gemSpec/gemSpec_VZD_FHIR_Directory/latest/[`gemSpec_VZD_FHIR_Directory`] |Normative Spezifikation des Dienstes (Anforderungen an Hersteller und Anbieter des VZD) |Verbindlich. Dieses Dokument macht keine abweichenden Vorgaben.
|https://github.com/gematik/api-vzd/blob/main/docs/gemILF_VZD_FHIR_Directory.adoc[`gemILF_VZD_FHIR_Directory`] |Implementierungsleitfaden für Clients |Verbindliche Grundlage für die Anbindung.
|https://github.com/gematik/api-vzd/blob/main/docs/FHIR_VZD_HOWTO_Search.adoc[`FHIR_VZD_HOWTO_Search.adoc`] |Technische Referenz der Suchschnittstelle inkl. Beispielrequests |*Primäre technische Quelle.* Dieses Dokument verweist darauf und ergänzt Empfehlungen zur Anwendung.
|https://github.com/gematik/api-vzd/blob/main/docs/FHIR_VZD_HOWTO_Authenticate.adoc[`FHIR_VZD_HOWTO_Authenticate.adoc`] |Beschaffung von `searchaccess_token` bzw. Token für `/fdv/search` |Voraussetzung für alle hier gezeigten Beispiele.
|https://simplifier.net/VZD-FHIR-Directory/~introduction[Datenmodell auf Simplifier (`vzd-fhir-directory`)] |Profile, CodeSystems, ValueSets |Referenz für alle Codes und Attribute.
...
|`/fdv/search` |Suchschnittstelle für Versichertenfrontends (FdV)
|`/search` |Suchschnittstelle für Nutzung durch Primärsysteme
...
image::click-dummy.png[Click-Dummy VZD-Suche im PVS,800]
...
|`/kim/search` |In Definition, geplant für VZD FHIR 1.2.0-19 |
...
|Kommunikationsadressen |`Endpoint` |über `_revinclude` / `_has` auflösbar
|===
...
[NOTE]
====
Dieses Kapitle beantwortet die in Interviews am häufigsten gestellte Frage: „Welche Suche nehme ich wann?“ Beispielhafte Ausgestaltung.
====
...
[NOTE]
====
*Neues Kapitel.* Relevant für ePA-/E-Rezept-Apps sowie für die Sortierung nach Entfernung.
====
